...
router.get('/stats', (req, res) => {
  ...
  const totalLeads = db.prepare('SELECT COUNT(*) as c FROM leads WHERE account_id = ? AND is_archived = 0 AND created_at >= ?').get(req.accountId, sinceStr).c
  const prevTotalLeads = db.prepare('SELECT COUNT(*) as c FROM leads WHERE account_id = ? AND is_archived = 0 AND created_at >= ? AND created_at < ?').get(req.accountId, prevSinceStr, sinceStr).c

  // Leads today
  const leadsToday = db.prepare("SELECT COUNT(*) as c FROM leads WHERE account_id = ? AND is_archived = 0 AND date(created_at) = date('now')").get(req.accountId).c
  ...
  const convData = db.prepare(`
    ...
    WHERE l.account_id = ? AND l.is_active = 1 AND l.is_archived = 0
  `).get(req.accountId)
  const conversionRate = convData.total > 0 ? (convData.converted / convData.total) * 100 : 0

  // Unassigned leads
  const unassigned = db.prepare('SELECT COUNT(*) as c FROM leads WHERE account_id = ? AND attendant_id IS NULL AND is_active = 1 AND is_archived = 0').get(req.accountId).c
  ...
  const bySource = db.prepare(`
    ...
    FROM leads WHERE account_id = ? AND is_archived = 0 AND created_at >= ?
...
router.get('/global', requireRole('super_admin'), (req, res) => {
  const accounts = db.prepare(`
    SELECT a.id, a.name, a.slug,
      (SELECT COUNT(*) FROM leads WHERE account_id = a.id AND is_archived = 0) as total_leads,
      (SELECT COUNT(*) FROM leads WHERE account_id = a.id AND is_archived = 0 AND date(created_at) = date('now')) as leads_today,
